...
let s:RE_TYPE_DECL	= '\(\(' .s:RE_TYPE_MODS. '\s\+\)*\)' .s:RE_TYPE_DECL_HEAD. '\(' .s:RE_IDENTIFIER. '\)[< \t\n\r]'
...
fu! s:CompleteAfterWord(incomplete)
  ...
  if b:context_type != s:CONTEXT_PACKAGE_DECL
    ...
    call cursor(1, 1)
    while 1
      let lnum = search('\(class\|interface\|enum\)[ \t\n\r]\+' . a:incomplete . '[a-zA-Z0-9_$]*[< \t\n\r]', 'W')
      if lnum == 0
	break
	...
	if text != ''
	  ...
	  if subs[2] =~# '^' . a:incomplete && (subs[0] =~ 'public\>' || fnamemodify(bufname(item.bufnr), ':p:h') == expand('%:p:h'))
...
function! s:GetStatement()
  if getCurrentLine() =~ '^\s*\(import\|package\)\s\+'
    return strpart(getCurrentLine(), match(getCurrentLine(), '\(import\|package\)'), col('.')-1)
  endif
...
function! s:GetThisClassDeclaration()
  ...
  while (1)
    call search('\(class\>\|interface\>\|enum\>\)[ \t\r\n]\+', 'bW')
    if !s:InComment(line("."), col(".")-1)
      if getCurrentLine()[col('.')-2] !~ '\S'
	break
      endif
    end
  endwhile
...
fu! s:DoGetTypeInfoForFQN(fqns, srcpath, ...)
  ...
  if !empty(dirs)
	...
	if text != ''
	  ...
	  if index(idents, subs[2]) >= 0 && (subs[0] =~ 'public\>' || dirpath == cwd)	" FIXME?
	    let item.subs = subs
